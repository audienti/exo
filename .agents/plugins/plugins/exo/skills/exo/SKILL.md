...
This plugin is a wrapper around the repo's real Exo CLI and onboarding logic. Do not invent a second onboarding flow.
...
```bash
node ../../scripts/init-exo-plugin.js --json
```
...
```bash
node ../../scripts/init-exo-plugin.js \
  --scope local-folder \
  --apply \
  --json
```
