...
principles:
  ...
  - Assign the company to one governed execution user before seeding more targets or writing more drafts.
...
avoid:
  ...
  - Do not treat a missing company assignment as a prospect-quality or message-quality problem.
writeback:
  - Persist the company-to-user assignment in Exo before you continue execution.
  - Keep the same execution user and browser identity for the whole company after you assign it.
---
Ready LinkedIn branches already exist, but they are blocked on missing execution ownership. Assign the blocked ready companies to {{userLabel}} first so Exo can turn the stored cadence branches and prospect context into live executable work today.
