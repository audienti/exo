...
function buildLinkedinCaptureOutputGuide(input) {
  ...
  return {
    ...
    surfaceStateRules: {
      ...
      visibleTotalCount: "Set visibleTotalCount to the full count reported by the connector or visibly shown by LinkedIn for that surface when one exists. If that visible count is smaller than the rows you itemized, treat the visible count as untrusted and set visibleTotalCount to itemCount. Otherwise use itemCount when the surface is fully exhausted or null when no trustworthy total is available.",
